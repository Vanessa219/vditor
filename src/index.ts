...
class Vditor extends VditorMethod {
  ...
  constructor(options?: IOptions) {
    ...
    if (!mergedOptions.i18n) {
      // i18n 配置项不存在
      if (
        !["en_US", "ja_JP", "ko_KR", "ru_RU", "zh_CN", "zh_TW"].includes(
          mergedOptions.lang
        )
      ) {
        throw new Error(
    ...
    addScript(
      ...
      this.vditor.lute = setLute({
        ...
        paragraphBeginningSpace:
          this.vditor.options.preview.markdown.paragraphBeginningSpace,
  ...
  public render(id: string | HTMLElement) {
    if (typeof id === "string") {
      //  挂载至DOM选择器渲染
      this.options.cache.id = `vditor${id}`
      id = document.getElementById(id) as HTMLElement
    }

    if (this.promises.length !== 0) {
      Promise.all(this.promises).then(() => {
        this.init(id as HTMLElement, this.options)
      })
    }
  }
}
