 ...
 contenteditable="true" spellcheck="false"></pre>`;
    ...
    private bindEvent(vditor: IVditor) {
        ...
        this.element.addEventListener(
            ...
            (event: MouseEvent & { target: HTMLInputElement }) => {
                ...
                if (previewElement) {
                    if (
                        ...
                        range.selectNodeContents(
                            previewElement.previousElementSibling
                                .firstElementChild
                        );
                    } else {
                        // 行内数学公式
                ...
                if (event.target.tagName === "IMG") {
                    const linkElement =
                        event.target.parentElement.querySelector<HTMLSpanElement>(
        ...
        this.element.addEventListener("keyup", (event) => {
            ...
            if (event.isComposing || this.composingLock || isCtrl(event)) {
                return;
            }
            if (event.key === "Enter") {
                scrollCenter(vditor);
